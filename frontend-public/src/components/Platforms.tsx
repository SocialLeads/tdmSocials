...
const Platforms: React.FC = () => (
  <section className="py-20 bg-white">
    <div className="max-w-6xl mx-auto px-6">
      <h2 className="text-3xl font-bold text-center text-gray-900 mb-4">Content voor elk platform</h2>
      <p className="text-center text-gray-500 mb-12">Elke dagelijkse e-mail bevat één op maat gemaakt contentidee per platform.</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-3xl mx-auto">
        {platforms.map((p) => (
          <div key={p.name} className={`text-center p-6 rounded-2xl border ${p.bg} transition-all shadow-sm hover:shadow-md`}>
            <p.Icon className={`w-10 h-10 mx-auto mb-3 ${p.iconColor}`} />
